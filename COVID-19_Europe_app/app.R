...
ui <- fluidPage(
        # Application title
        titlePanel(title = h3("COVID-19: Comparing European countries per week", 
                              align="center")),
...
server <- function(input, output) {
        ...
        #plotting Iets klopt nog niet helemaal
                ...
                output$deathsPlot <- renderPlot({
                        deaths_sub <- selectedData()
                        plot(deaths_sub[, 1], type = "l", 
                             ylab = "Deaths",
                             xlab = "Weeknumber",
                             main = "Comparing countries on COVID-19 deaths per week",
                             xlim = input$weeks,
                             ylim = c(0, max(c(max(deaths_sub[, 1]), max(deaths_sub[, 2])))),
                             col = "red")
                        lines(deaths_sub[, 2], col = "blue")
                })
        }
